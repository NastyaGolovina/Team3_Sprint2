...
public interface RouteLineRepository extends JpaRepository<RouteLine, String> {
	...
	@Query(value = "SELECT COUNT(*) > 0 FROM team_3.routeline WHERE countrySenderId = ?1 OR countryReceiverId = ?1", nativeQuery = true)
	boolean existsByCountryId(String countryId);

	@Query(value = "SELECT COUNT(*) > 0 FROM team_3.routeline WHERE transportId = ?1", nativeQuery = true)
	boolean existsByTransportId(String transportId);

	@Query(value = "SELECT COUNT(*) > 0 FROM team_3.routeline WHERE originSiteId = ?1 OR destinationSiteId = ?1", nativeQuery = true)
	boolean getRouteLineBySiteId(String siteId);

	@Query(value = "SELECT COUNT(*) > FROM team_3.routeline WHERE productId = ?1", nativeQuery = true)
